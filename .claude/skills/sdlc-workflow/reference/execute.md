...
| `.claude/skills/sdlc-knowledge/reference/context-packs/02,06,07,08,09` | Engineering standards, test strategy, security/privacy, ops readiness, release governance |
| `.claude/skills/sdlc-knowledge/reference/templates/{implementation-plan,test-plan,validation-report,release-plan,pr-package,post-implementation-review}.md` | Artifact shapes |
| `.claude/skills/sdlc-knowledge/reference/autonomy/escalation-policy.md` | Hard-stop triggers (production deploy is always one) |
...
7. **Run `/sdlc:9-learning`**
   ...
   - **Fold lessons back into the framework** by editing files under `.claude/skills/sdlc-knowledge/reference/`:
     - new patterns/constraints/edge cases → `context-packs/`
     - new hard-stop triggers learned from near-misses → `autonomy/escalation-policy.md`
     - tighter/lighter gates → `quality-gates/`
     - a wrongly-classified change class → `agent/change-classes.md`
